...
.defines(function() {
    EntityPlayer = ig.Entity.extend({
        ...
        turnCounter: 0,
        tileSize: 0,
        imgGreen: new ig.Image('media/green.png'),
        ...
        draw: function() {
            ...
            {
                // left
                x = this.pos.x - (this.tileSize * (i + 1));
                y = this.pos.y;
                this.imgGreen.drawTile(x, y, i, 10);
                ...
                // up
                x = this.pos.x;
                y = this.pos.y - (this.tileSize * (i + 1));
                this.imgGreen.drawTile(x, y, i, 10);
            ...
            */
            ...
            /**/
            // draw a sqaure of green around the destination
            var x = 0;
            var y = 0;
            for (var i = -5; i < 4; i++)
            {
                for (var j = -5; j < 4; j++)
                {
                    x = this.destination.x + (this.tileSize * (i + 1));
                    y = this.destination.y + (this.tileSize * (j + 1));
                    if (ig.game.collisionMap.getTile(x+5, y+5) == 0)
                    {
                        this.imgGreen.drawTile(x, y, 1, 10);
                    }
                    else
                    {
                        this.imgGreen.drawTile(x, y, 3, 10);
                    }
                }
            }
            /**/
        ...
        isMoving: function() {
            ...
            else
            {
                ...
                if (ig.input.state('left') && ! this.checkForTile(-1, 0))
                {
                ...
                if (ig.input.state('right') && ! this.checkForTile(1, 0))
                {
                    this.vel.x += this.speed;
                    this.destination.x = (this.pos.x + this.tileSize);
                    this.moving = true;
                ...
                if (ig.input.state('up') && ! this.checkForTile(0, -1))
                {
                ...
                if (ig.input.state('down') && ! this.checkForTile(0, 1))
                {
                    this.vel.y += this.speed;
                    this.destination.y = (this.pos.y + this.tileSize);
                    this.moving = true;
